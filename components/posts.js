...
export function getAllRecipesFilesIds() {
  const fileNames = fs.readdirSync(recipesFilesDirectory);
  return fileNames.map((fileName) => {
    return {
      params: {
        id: fileName.replace(/\.md$/, ""),
...
export async function getRecipesFilesData(id) {
  const fullPath = path.join(recipesFilesDirectory, `${id}.md`);
  const fileContents = fs.readFileSync(fullPath, "utf8");
...
export function getSortedRecipesFilesData() {
  // Get file names under /posts
  const fileNames = fs.readdirSync(recipesFilesDirectory);
  console.log(`fileNames: ${fileNames}`);
  const allPostsData = fileNames.map((fileName) => {
    ...
    // Read markdown file as string
    const fullPath = path.join(recipesFilesDirectory, fileName);
    const fileContents = fs.readFileSync(fullPath, "utf8");
